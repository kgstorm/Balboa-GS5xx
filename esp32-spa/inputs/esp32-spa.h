...
class HotTubDisplaySensor : public esphome::Component, public esphome::sensor::Sensor {
 public:
  ...
  void setup() override {
    ...
    // Press TEMP/LIGHTS three times after boot to capture the mode and cycle it back to the original setting.
    // Delay all startup actions until after startup ignore window.
    this->set_timeout("boot_press_temp_on", STARTUP_IGNORE_MS + 5000, [this]() {
      ESP_LOGI(TAG, "Boot: auto-pressing TEMP to initialize set temp");
      gpio_set_level((gpio_num_t)PIN_WRITE_BTN2, 1);
      last_set_sent_time_ms = esphome::millis();
    });
    this->set_timeout("boot_press_temp_off", STARTUP_IGNORE_MS + 5200, []() {
      gpio_set_level((gpio_num_t)PIN_WRITE_BTN2, 0);
    });
    ...
    this->set_timeout("boot_press_temp_on_2", STARTUP_IGNORE_MS + 8000, []() {
      gpio_set_level((gpio_num_t)PIN_WRITE_BTN2, 1);
    });
    this->set_timeout("boot_press_temp_off_2", STARTUP_IGNORE_MS + 8200, []() {
      gpio_set_level((gpio_num_t)PIN_WRITE_BTN2, 0);
    });
    ...
    this->set_timeout("boot_press_temp_on_3", STARTUP_IGNORE_MS + 11000, []() {
      gpio_set_level((gpio_num_t)PIN_WRITE_BTN2, 1);
    });
    this->set_timeout("boot_press_temp_off_3", STARTUP_IGNORE_MS + 11200, []() {
      gpio_set_level((gpio_num_t)PIN_WRITE_BTN2, 0);
    });
